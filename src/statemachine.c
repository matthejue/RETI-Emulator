...
void update_state(Event event) { update_state_with_io(event, NULL, NULL); }

void update_state_with_io(Event event, struct StateInput *in,
                          struct StateOutput *out) {
  switch (current_state) {
  case HI:
    // HI -> NB (event = RETURN_FROM_INTERRUPT | !si_happened && stack_top==0 &&
    // heap_size==0 | isr_active=false; stack_top--; return_from_interrupt();
    // finish_if_finished_here(); step_out_if_stepped_into_here();)
    if (event == RETURN_FROM_INTERRUPT && !si_happened && stack_top == 0 &&
        heap_size == 0) {
      current_state = NB;
      isr_active = false;
      stack_top--;
    ...
    // HI -> HI (event = HARDWARE_INTERRUPT |
    // (out->retbool1=check_prio_isr(in->arg8)) | interrupt_timer_active=false;
    // stack_top++; isr_priority_stack[stack_top]=isr_to_prio[arg];
    // display_notification_box_with_action(); setup_interrupt(arg);)
    if (event == HARDWARE_INTERRUPT &&
    ...
    // HI -> HI (event = RETURN_FROM_INTERRUPT | heap_size>1 &&
    // !check_prio_heap() | stack_top--; return_from_interrupt();
    // finish_if_finished_here(); step_out_if_stepped_into_here();)
    ...
    // HI -> SI (event = RETURN_FROM_INTERRUPT | si_happened && stack_top==0 &&
    // heap_size==0 | stack_top--; return_from_interrupt();
    // finish_if_finished_here(); step_out_if_stepped_into_here();)
    if (event == RETURN_FROM_INTERRUPT && si_happened && stack_top == 0 &&
        heap_size == 0) {
      current_state = SI;
    ...
    // interrupt_timer_active = false; insert_into_heap();)
    if (event == HARDWARE_INTERRUPT &&
        !(out->retbool1 = check_prio_isr(in->arg8)) && heap_size < HEAP_SIZE) {
      current_state = HI;
      interrupt_timer_active = false;
    ...
    // HI -> HI (event = RETURN_FROM_INTERRUPT | heap_size>1 &&
    // check_prio_heap() | return_from_interrupt(); heap_size--;
    // step_out_if_stepped_into_here(); handle_next_hi();)
    if (event == RETURN_FROM_INTERRUPT && heap_size > 1 && check_prio_heap()) {
      current_state = HI;
      return_from_interrupt();
      heap_size--;
    ...
    // HI -> HI (event = FINALIZE | isr_finished && isr_active |
    // finished_here=stack_top; isr_finished=false;)
    if (event == FINALIZE && isr_finished && isr_active) {
      current_state = HI;
      started_finish_here = stack_top;
      isr_finished = false;
    }
    // HI -> SI (event = RETURN_FROM_INTERRUPT | si_happened && stack_top==-1 &&
    // heap_size==1 | heap_size--; return_from_interrupt();
    // finish_if_finished_here(); step_out_if_stepped_into_here();)
    if (event == RETURN_FROM_INTERRUPT && si_happened && stack_top == -1 &&
        heap_size == 1) {
      current_state = SI;
      heap_size--;
      return_from_interrupt();
    ...
    // HI -> NB (event = RETURN_FROM_INTERRUPT | !si_happened && stack_top==-1
    // && heap_size==1 | isr_active=false; heap_size--; return_from_interrupt();
    // finish_if_finished_here(); step_out_if_stepped_into_here();)
    if (event == RETURN_FROM_INTERRUPT && !si_happened && stack_top == -1 &&
        heap_size == 1) {
      current_state = NB;
    ...
    // HI -> HI (event = SOFTWARE_INTERRUPT |  | interrupt_timer_active = false;
  ...
  case NB:
    // NB -> SI (event = SOFTWARE_INTERRUPT |  | isr_active=true;
    // si_happened=true; setup_interrupt();)
    if (event == SOFTWARE_INTERRUPT) {
      current_state = SI;
      isr_active = true;
      si_happened = true;
    ...
    // NB -> SI (event = STEP_INTO_ACTION | check_if_int_i() |
    // step_into_activated=true; stepped_into_here=stack_top; isr_active=true;
    // si_happened=true; setup_interrupt();)
    ...
    // NB -> HI (event = HARDWARE_INTERRUPT | stack_top==-1 |
  ...
  case SI:
    // SI -> NB (event = RETURN_FROM_INTERRUPT |  | isr_active=false;
    // si_happened=false; return_from_interrupt(); finish_if_finished_here();
    ...
    // SI -> HI (event = HARDWARE_INTERRUPT | stack_top==-1 |
    // check_deactivation_interrupt_timer(in->arg8);
    // setup_interrupt_for_hi(in->arg8, &out->retbool1);)
    if (event == HARDWARE_INTERRUPT && stack_top == -1) {
      current_state = HI;
