...
- `<esc><path><esc>/` leitet alle folgenden UART-Ausgabebytes in die neu angelegte beziehungsweise geleerte Datei um. `<esc>stdout<esc>/` schaltet zurück zur Terminalausgabe, `<esc>stderr<esc>/` leitet auf die Standardfehlerausgabe um.
